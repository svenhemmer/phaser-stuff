...
export class ExportPrompt {
    constructor(_doc) {
        ...
        this._formatSelect = select({ style: "width: 100%;" }, option({ value: "wav" }, ".wav"), option({ value: "mp3" }, ".mp3"), option({ value: "midi" }, ".mid"), option({ value: "json" }, ".json (for any BeepBox version)"), option({ value: "html" }, ".html (opens BeepBox)"));
        this._cancelButton = button({ class: "cancelButton" });
        this._exportButton = button({ class: "exportButton", style: "width:45%;" }, "Export");
        this.container = div({ class: "prompt noSelection", style: "width: 200px;" }, h2("Export Options"), div({ style: "display: flex; flex-direction: row; align-items: center; justify-content: space-between;" }, "File name:", this._fileName), div({ style: "display: table; width: 100%;" }, div({ style: "display: table-row;" }, div({ style: "display: table-cell;" }, "Intro:"), div({ style: "display: table-cell;" }, "Loop Count:"), div({ style: "display: table-cell;" }, "Outro:")), div({ style: "display: table-row;" }, div({ style: "display: table-cell; vertical-align: middle;" }, this._enableIntro), div({ style: "display: table-cell; vertical-align: middle;" }, this._loopDropDown), div({ style: "display: table-cell; vertical-align: middle;" }, this._enableOutro))), div({ style: "text-align: left;" }, "File Type:"), div({ class: "selectContainer", style: "width: 100%;" }, this._formatSelect), div({ style: "text-align: left;" }, "(Be patient, exporting may take some time...)"), div({ style: "display: flex; flex-direction: row-reverse; justify-content: space-between;" }, this._exportButton), this._cancelButton);
        this._close = () => {
            this._doc.undo();
        };
        this.cleanUp = () => {
